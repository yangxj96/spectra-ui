...
const row = defineModel<DictGroup>("row");

defineModel<DictGroup>("group");
...
// 定义字典组编辑对话框的状态和表单数据
const edit = reactive({
    title: computed(() => (has_edit.value ? "编辑" : "新增") + "字典组"),
    rules: {
        name: [
...
onMounted(() => {
    handleInitData();
    edit.form = has_edit.value ? JSON.parse(JSON.stringify(row.value || edit.form)) : ({ state: 0 } as DictGroup);
});
